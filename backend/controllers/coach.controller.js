...
exports.findOne = (req, res) => {
    ...
    Coach.findOne({'name': req.query.name})
        .then(coach => {
            console.log(coach);
            res.send(coach);
        })
        .catch(err => {
            res.status(500).send({
                message: err.message || `Cannot find coach ${req.query.name}`
            });
        });
};
...
exports.getTeamsForCoach = (req, res) => {
    ...
    Coach.findById(req.query.coachId)
        .then(coach => {
            console.log(coach.teams);
        ...
        .catch(err => {
            res.status(500).send({
                message: err.message || `Cannot find coach with id ${req.query.coachId}`
            });
        });
};
...
exports.getTeamsForCoach = (req, res) => {
    ...
    Coach.findOne({'name': req.query.name})
        .then(coach => {
            console.log(coach.teams.length);
            res.send(coach.teams);
        })
        .catch(err => {
            res.status(500).send({
                message: err.message || `Cannot find coach with id ${req.query.name}`
            });
        });
};*/
